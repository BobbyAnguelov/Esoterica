...
int texconv_main( _In_ int argc, _In_z_count_( argc ) wchar_t* argv[], Blob& outBlob )
{
    ...
    for ( auto pConv = conversion.begin(); pConv != conversion.end(); ++pConv )
    {
        ...
        // --- Save result -------------------------------------------------------------
        {
            ...
            switch ( FileType )
            {
                ...
                case CODEC_PPM:
                //hr = SaveToPortablePixMap( img[0], outBlob );
                break;

                case CODEC_PFM:
                //hr = SaveToPortablePixMapHDR( img[0], outBlob );
                break;
...
namespace EE::Render
{
    bool ConvertTexture( FileSystem::Path const& texturePath, TextureType type, Blob& rawData )
    {
        ...
        if ( texconv_main( numArgs, args, textureBlob ) == 0 )
        {
            // Error occurred
