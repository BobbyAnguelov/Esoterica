...
async function main() {
    ...
    function memString(ptr, length) {
        ...
        const strData = data8.slice(Number(ptr), Number(ptr + length))
    ...
    function allocString(str) {
        ...
        const arr = new Uint8Array(mem(), Number(ptr), utf8.length + 1)
    ...
    const wasmModule = await WebAssembly.instantiate(wasmData, {
        ...
        host: {
            ...
            hostReadFile: (index, dst, offset, count) => {
                const dstSlice = new Uint8Array(mem(), Number(dst), Number(count))
                const srcSlice = openFiles[index].slice(Number(offset), Number(offset + count))
            ...
            hostDump: (data, length) => {
                const slice = new Uint8Array(mem(), Number(data), Number(length))
    ...
    function hashAlloc(size) {
        return wasmModule.instance.exports.hashAlloc(BigInt(size))
    }
    ...
    async function hashScene(filename, frame, dumpFile) {
        ...
        const dataSlice = new Uint8Array(mem(), Number(dataPtr), data.length)
        dataSlice.set(data)

        const filenamePtr = allocString(filename)

        new DataView(mem()).setBigUint64(Number(resultPtr), 0n, true)

        const returnCode = wasmModule.instance.exports.hashScene(resultPtr, dataPtr, BigInt(data.length), filenamePtr, frame, dumpFile ? 0xDFn : 0n)
        ...
        const hash = new DataView(mem()).getBigUint64(Number(resultPtr), true)
