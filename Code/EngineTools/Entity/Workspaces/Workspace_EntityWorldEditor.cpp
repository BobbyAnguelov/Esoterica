...
{
    EntityWorldEditorWorkspace::EntityWorldEditorWorkspace( ToolsContext const* pToolsContext, EntityWorld* pWorld, ResourceID const& resourceID )
        : Workspace( pToolsContext, pWorld, resourceID )
        , m_context( pToolsContext, pWorld, m_undoStack )
        , m_worldOutliner( pToolsContext, m_context )
        , m_entityEditor( pToolsContext, m_context )
    {
        m_gizmo.SetTargetTransform( &m_gizmoTransform );
    }

    EntityWorldEditorWorkspace::EntityWorldEditorWorkspace( ToolsContext const* pToolsContext, EntityWorld* pWorld, String const& displayName )
    ...
    void EntityWorldEditorWorkspace::Initialize( UpdateContext const& context )
    {
        Workspace::Initialize( context );
        m_entityEditor.Initialize( context, GetID() );
        m_worldOutliner.Initialize( context, GetID() );
    ...
    void EntityWorldEditorWorkspace::Shutdown( UpdateContext const& context )
    {
        m_worldOutliner.Shutdown( context );
        m_entityEditor.Shutdown( context );
        Workspace::Shutdown( context );
    }

    void EntityWorldEditorWorkspace::InitializeDockingLayout( ImGuiID dockspaceID ) const
    {
        ...
        ImGui::DockBuilderDockWindow( m_worldOutliner.GetWindowName(), bottomLeftDockID );
        ImGui::DockBuilderDockWindow( m_entityEditor.GetPropertyGridWindowName(), bottomCenterDockID );
        ImGui::DockBuilderDockWindow( m_entityEditor.GetEntityStructureWindowName(), bottomRightDockID );
    }

    //-------------------------------------------------------------------------

    void EntityWorldEditorWorkspace::PostUndoRedo( UndoStack::Operation operation, IUndoableAction const* pAction )
    {
        m_context.OnUndoRedo( operation, pAction );
    }

    void EntityWorldEditorWorkspace::DropResourceInViewport( ResourceID const& resourceID, Vector const& worldPosition )
    {
    ...
    void EntityWorldEditorWorkspace::OnMousePick( Render::PickingID pickingID )
    {
    ...
    void EntityWorldEditorWorkspace::DrawViewportToolbarItems( UpdateContext const& context, Render::Viewport const* pViewport )
    {
    ...
    void EntityWorldEditorWorkspace::DrawViewportOverlayElements( UpdateContext const& context, Render::Viewport const* pViewport )
    {
        auto drawingCtx = GetDrawingContext();
        auto pEditedMap = m_context.GetMap();

        {
            ImGuiX::ScopedFont const sf( ImGuiX::Font::LargeBold, Colors::Red );
            ImGui::Indent( 20 );
            for ( auto i = 0; i < 5; i++ )
            {
                ImGui::NewLine();
                ImGui::Text( "ENTITY MAP EDITING TEMPORARILY DISABLED AS IT IS BEING OVERHAULED!!!" );
            }
            ImGui::Unindent();
        }
    ...
    void EntityWorldEditorWorkspace::Update( UpdateContext const& context, ImGuiWindowClass* pWindowClass, bool isFocused )
    {
        ...
        //    if ( hack && m_context.GetMap() != nullptr )
        //    {
        //        m_worldOutliner.SetWorldToOutline( m_context.GetWorld(), m_context.GetMap()->GetID() );
        //        hack = false;
        //    }
        //}

        //m_worldOutliner.UpdateAndDraw( context, pWindowClass );
        //m_entityEditor.UpdateAndDraw( context, pWindowClass );
    }
}