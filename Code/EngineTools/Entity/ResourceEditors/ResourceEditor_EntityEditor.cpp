...
namespace EE::EntityModel
{
    ...
    public:
        ...
        EntityUndoableAction( EntityEditor* pEditor ) 
            : m_pEditor( pEditor )
        {
        ...
        EntityEditor::SelectionSwitchRequest const& GetRecordedSelection() const { return m_selection; }
    ...
    private:
        ...
        EntityEditor*                                       m_pEditor = nullptr;
        Type                                                m_actionType = Invalid;
        EntityEditor::SelectionSwitchRequest                m_selection;
    ...
    {
        EntityWorld* pWorld = m_pEditor->m_pWorld;
    ...
    void EntityUndoableAction::RecordSelection()
    {
        ...
        if ( m_pEditor->m_outlinerTreeView.HasSelection() )
        {
            for ( auto pSelectedItem : m_pEditor->m_outlinerTreeView.GetSelection() )
            {
                m_selection.m_selectedEntities.emplace_back( EntityID( pSelectedItem->GetUniqueID() ) );
            }

            for ( auto pSelectedItem : m_pEditor->m_structureEditorTreeView.GetSelection() )
            {
...
namespace EE::EntityModel
{
    ...
    void EntityEditor::Initialize( UpdateContext const& context )
    {
    ...
    void EntityEditor::InitializeDockingLayout( ImGuiID dockspaceID, ImVec2 const& dockspaceSize ) const
    {
    ...
    void EntityEditor::DrawViewportToolbar( UpdateContext const& context, Render::Viewport const* pViewport )
    {
    ...
    void EntityEditor::DropResourceInViewport( ResourceID const& resourceID, Vector const& worldPosition )
    {
    ...
    void EntityEditor::OnMousePick( Render::PickingID pickingID )
    {
        if ( !pickingID.IsSet() )
        {
    ...
    void EntityEditor::UpdateSelectionSpatialInfo()
    {
        TVector<OBB> bounds;
    ...
    void EntityEditor::RequestDestroyEntity( Entity* pEntity )
    {
    ...
    void EntityEditor::RequestDestroyEntities( TVector<Entity*> entities )
    {
    ...
    void EntityEditor::RequestEntityReparent( Entity* pEntity, Entity* pNewParentEntity )
    {
        EE_ASSERT( pEntity != nullptr && pEntity->IsAddedToMap() && m_pWorld->FindEntity( pEntity->GetID() ) != nullptr );
    ...
    void EntityEditor::StartAddComponentOrSystem( AddRequestType addRequestType, EntityComponent* pTargetComponent /*= nullptr */ )
    {
    ...
    void EntityEditor::CreateEntity( EntityMapID const& mapID )
    {
    ...
    TVector<Entity*> EntityEditor::DuplicateSelectedEntities()
    {
        EE_ASSERT( !m_outlinerTreeView.IsCurrentlyDrawingTree() ); // This function rebuilds the tree so it cannot be called during tree drawing
    ...
    void EntityEditor::RenameEntity( Entity* pEntity, StringID newDesiredName )
    {
        EE_ASSERT( !m_outlinerTreeView.IsCurrentlyDrawingTree() ); // This function rebuilds the tree so it cannot be called during tree drawing
    ...
    void EntityEditor::SetEntityParent( Entity* pEntity, Entity* pNewParent )
    {
        EE_ASSERT( !m_outlinerTreeView.IsCurrentlyDrawingTree() ); // This function rebuilds the tree so it cannot be called during tree drawing
    ...
    void EntityEditor::ClearEntityParent( Entity* pEntity )
    {
        EE_ASSERT( !m_outlinerTreeView.IsCurrentlyDrawingTree() ); // This function rebuilds the tree so it cannot be called during tree drawing
    ...
    void EntityEditor::CreateSystem( TypeSystem::TypeInfo const* pSystemTypeInfo )
    {
    ...
    void EntityEditor::DestroySystem( TypeSystem::TypeID systemTypeID )
    {
        auto pEditedEntity = GetEditedEntity();
    ...
    void EntityEditor::DestroySystem( EntitySystem* pSystem )
    {
        auto pEditedEntity = GetEditedEntity();
    ...
    void EntityEditor::CreateComponent( TypeSystem::TypeInfo const* pComponentTypeInfo, ComponentID const& parentSpatialComponentID )
    {
        auto pEditedEntity = GetEditedEntity();
    ...
    void EntityEditor::DestroyComponent( EntityComponent* pComponent )
    {
        auto pEditedEntity = GetEditedEntity();
        EE_ASSERT( pComponent != nullptr );
        EE_ASSERT( pEditedEntity != nullptr );
    ...
    void EntityEditor::RenameComponent( EntityComponent* pComponent, StringID newNameID )
    {
        auto pEditedEntity = GetEditedEntity();
        EE_ASSERT( pComponent != nullptr );
        EE_ASSERT( pEditedEntity != nullptr );
    ...
    void EntityEditor::ReparentComponent( SpatialEntityComponent* pComponent, SpatialEntityComponent* pNewParentComponent )
    {
        auto pEditedEntity = GetEditedEntity();
        EE_ASSERT( pEditedEntity != nullptr );
    ...
    void EntityEditor::MakeRootComponent( SpatialEntityComponent* pComponent )
    {
        auto pEditedEntity = GetEditedEntity();
        EE_ASSERT( pEditedEntity != nullptr );
        EE_ASSERT( pComponent != nullptr );
    ...
    EntityMap* EntityEditor::GetEditedMap()
    {
        return m_pWorld->GetFirstNonPersistentMap();
    }

    TVector<Entity*> EntityEditor::GetSelectedSpatialEntities() const
    {
    ...
    TVector<Entity*> EntityEditor::GetSelectedEntities() const
    {
    ...
    void EntityEditor::DrawOutliner( UpdateContext const& context, bool isFocused )
    {
        // Draw Tree
        //-------------------------------------------------------------------------
        ...
        if ( isFocused )
        {
            ...
            if ( ImGui::IsKeyPressed( ImGuiKey_F2 ) )
            {
                if( selection.size() == 1 )
                {
                    ...
                    m_dialogManager.CreateModalDialog( "Rename Entity", [this] ( UpdateContext const& context ) { return DrawRenameEntityDialog( context ); }, ImVec2( 500, 120 ) );
                }
            }
    ...
    void EntityEditor::RebuildOutlinerTree( TreeListViewItem* pRootItem )
    {
        EntityMap* pMap = GetEditedMap();
        if ( pMap == nullptr )
        {
    ...
    void EntityEditor::DrawOutlinerContextMenu( TVector<TreeListViewItem*> const& selectedItemsWithContextMenus )
    {
    ...
    void EntityEditor::HandleOutlinerDragAndDrop( TreeListViewItem* pDropTarget )
    {
    ...
    Entity* EntityEditor::GetEditedEntity() const
    {
    ...
    TVector<SpatialEntityComponent*> EntityEditor::GetSelectedSpatialComponents() const
    {
    ...
    void EntityEditor::DrawStructureEditor( UpdateContext const& context, bool isFocused )
    {
    ...
    void EntityEditor::RebuildStructureEditorTree( TreeListViewItem* pRootItem )
    {
        Entity* pEditedEntity = GetEditedEntity();
        if ( pEditedEntity == nullptr )
        {
    ...
    void EntityEditor::HandleStructureEditorDragAndDrop( TreeListViewItem* pDropTarget )
    {
    ...
    void EntityEditor::DrawPropertyGrid( UpdateContext const& context, bool isFocused )
    {
        ...
        if ( ImGui::BeginChild( "ST", ImVec2( -1, 24 ), ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_NoMove ) )
        {
    ...
    bool EntityEditor::DrawRenameEntityDialog( UpdateContext const& context )
    {
        ...
        if ( ImGui::InputText( "##Name", m_operationBuffer, 256, ImGuiInputTextFlags_EnterReturnsTrue | ImGuiInputTextFlags_CharsNoBlank | ImGuiInputTextFlags_CallbackCharFilter, ImGuiX::FilterNameIDChars ) )
        {
            applyRename = true;
            isDialogOpen = false;
        }
    ...
    bool EntityEditor::DrawRenameComponentDialog( UpdateContext const& context )
    {
        Entity* pEditedEntity = GetEditedEntity();
    ...
    bool EntityEditor::DrawAddComponentOrSystemDialog( UpdateContext const& context )
    {
        bool isDialogOpen = true;
    ...
    void EntityEditor::BeginTransformManipulation( Transform const& newTransform, bool duplicateSelection )
    {
    ...
    void EntityEditor::ApplyTransformManipulation( Transform const& newTransform )
    {
        EE_ASSERT( m_pActiveUndoableAction != nullptr );
    ...
    void EntityEditor::EndTransformManipulation( Transform const& newTransform )
    {
        ApplyTransformManipulation( newTransform );
    ...
    void EntityEditor::Update( UpdateContext const& context, bool isVisible, bool isFocused )
    {
        // Process deletions
        //-------------------------------------------------------------------------
