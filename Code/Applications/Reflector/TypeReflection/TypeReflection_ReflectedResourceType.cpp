...
namespace EE::Reflection
{
    bool ReflectedResourceType::TryParseResourceRegistrationMacroString( String const& registrationStr )
    {
        ...
        size_t const resourceColorNameStartIdx = registrationStr.find_first_of( ',', resourceFriendlyNameEndIdx + 1 );
        if ( resourceColorNameStartIdx == String::npos )
        {
            return false;
        }
        size_t const resourceColorNameEndIdx = registrationStr.find_first_of( ',', resourceColorNameStartIdx + 1 );
        if ( resourceColorNameEndIdx == String::npos )
        {
            return false;
        }
        ...
        EE_ASSERT( resourceColorNameStartIdx != resourceColorNameEndIdx );
        ...
        m_color = registrationStr.substr( resourceColorNameStartIdx + 1, resourceColorNameEndIdx - resourceColorNameStartIdx - 1 );
        m_color = StringUtils::StripAllWhitespace( m_color );
