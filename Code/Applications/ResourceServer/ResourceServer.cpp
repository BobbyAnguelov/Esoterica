...
namespace EE::Resource
{
    ...
    bool ResourceServer::Initialize( IniFile const& iniFile )
    {
        ...
        {
            m_workers.emplace_back( EE::New<ResourceServerWorker>( &m_taskSystem, m_context, m_settings.m_resourceCompilerExecutablePath.c_str() ) );
        }
    ...
    void ResourceServer::Update()
    {
        ...
        while ( m_pendingRequests.size() > 0 && m_activeRequests.size() < m_maxSimultaneousCompilationTasks )
        {
            ...
            for ( auto& pWorker : m_workers )
            {
                if ( pWorker->IsIdle() )
                {
                    pWorker->ProcessRequest( pRequestToStart );
                    taskStarted = true;
                    break;
                }
            }
