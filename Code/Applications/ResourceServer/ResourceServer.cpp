    ...
    void ResourceServer::CreateResourceRequest( ResourceID const& resourceID, uint32_t clientID, CompilationRequest::Origin origin )
    {
        ...
        if ( resourceID.IsValid() )
        {
            ...
            else
            {
                ...
                CompileDependencyTree compileDependencyTree( m_settings.m_rawResourcePath, m_settings.m_compiledResourcePath, *m_pCompilerRegistry, m_compiledResourceDatabase );
                if ( compileDependencyTree.BuildTree( pRequest->m_resourceID ) )
                {
                    CompileDependencyNode const* pRoot = compileDependencyTree.GetRoot();
                    pRequest->m_compilerVersion = pRoot->m_compilerVersion;
                    pRequest->m_fileTimestamp = pRoot->m_timestamp;
                    pRequest->m_sourceTimestampHash = pRoot->m_combinedHash;
                    pRequest->m_status = pRoot->IsUpToDate() ? CompilationRequest::Status::SucceededUpToDate : CompilationRequest::Status::Pending;
                }
                else // Failed to generate dependency tree
                {
                    pRequest->m_log.sprintf( compileDependencyTree.GetErrorMessage().c_str() );
                    pRequest->m_status = CompilationRequest::Status::Failed;
                }
                ...
                if ( pRequest->HasSucceeded() && forceRecompile )
                {
