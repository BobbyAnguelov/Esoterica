...
namespace EE
{
    constexpr static float      g_maxSprintSpeed = 18.0f;                      // meters/second
    constexpr static float      g_maxRunSpeed = 6.0f;                         // meters/second
    ...
    static float ConvertStickAmplitudeToSpeed( PlayerActionContext const& ctx, float stickAmplitude )
    {
        float const baseSpeed = ctx.m_pPlayerState->m_sprintFlag ? g_maxSprintSpeed : ctx.m_pPlayerState->m_crouchFlag ? g_maxCrouchSpeed : g_maxRunSpeed;
        return stickAmplitude * baseSpeed;
    }

    //-------------------------------------------------------------------------

    bool PlayerAction_Move::TryStartInternal( PlayerActionContext const& ctx )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();

        ctx.m_pPlayer->SetGravityScale( 0.9f );

        ctx.m_pAnimationController->SetCharacterState( PlayerAnimationController::CharacterState::Locomotion );
        SetCrouchState( ctx, ctx.m_pPlayerState->m_crouchFlag );
    ...
    PlayerAction::Status PlayerAction_Move::UpdateInternal( PlayerActionContext const& ctx, bool isFirstUpdate )
    {
        // Process inputs
        //-------------------------------------------------------------------------
        ...
        /*if ( ctx.m_pPlayerController->GetFloorType() != CharacterPhysicsController::FloorType::Navigable && ctx.m_pPlayer->GetVelocity().GetZ() < -Math::Epsilon )
        {
            ...
            {
                isSliding = true;
                m_desiredFacing = ctx.m_pPlayer->GetVelocity().GetNormalized2();
            }
        }
        else
        {
        ...
        ctx.m_pAnimationController->SetCrouch( ctx.m_pPlayerState->m_crouchFlag );
        ...
        if ( m_areVisualizationsEnabled )
        {
            ...
            auto drawingCtx = ctx.GetDebugDrawContext();
    ...
    void PlayerAction_Move::StopInternal( PlayerActionContext const& ctx, StopReason reason )
    {
        ctx.m_pPlayerState->m_sprintFlag = false;
    }

    void PlayerAction_Move::SetCrouchState( PlayerActionContext const& ctx, bool isCrouchEnabled )
    {
        // Enable crouch
        if ( isCrouchEnabled )
        {
            ctx.m_pPlayer->ResizeCapsule( ctx.m_pPlayer->GetCapsuleRadius(), 0.2f );
            ctx.m_pPlayerState->m_crouchFlag = true;
        }
        else // Disable crouch
        {
            ctx.m_pPlayer->ResetCapsuleSize();
            ctx.m_pPlayerState->m_crouchFlag = false;
        }
    }

    //-------------------------------------------------------------------------

    void PlayerAction_Move::RequestIdle( PlayerActionContext const& ctx )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();
    ...
    void PlayerAction_Move::UpdateIdle( PlayerActionContext const& ctx, Vector const& stickInputVectorWS, float stickAmplitude )
    {
        ...
        {
            Radians const deltaAngle = Math::CalculateAngleBetweenVectors( characterForward, stickInputVectorWS );
            if ( deltaAngle < g_idle_immediateStartThresholdAngle )
            {
                //RequestStart( ctx, stickInputVectorWS, stickAmplitude );
                RequestMoving( ctx, stickInputVectorWS );
            }
            else // turn on spot
            {
    ...
    void PlayerAction_Move::RequestTurnOnSpot( PlayerActionContext const& ctx, Vector const& desiredFacingDirection )
    {
        m_desiredMovementVelocity = Vector::Zero;
        m_cachedFacing = ctx.m_pPlayer->GetForwardVector();
    ...
    void PlayerAction_Move::UpdateTurnOnSpot( PlayerActionContext const& ctx, Vector const& stickInputVectorWS, float stickAmplitude )
    {
        ...
        if ( m_areVisualizationsEnabled )
        {
            Vector const characterPosition = ctx.m_pPlayer->GetWorldTransform().GetTranslation();
            auto drawingCtx = ctx.GetDebugDrawContext();
    ...
    void PlayerAction_Move::RequestStart( PlayerActionContext const& ctx, Vector const& stickInputVector, float stickAmplitude )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();
    ...
    void PlayerAction_Move::UpdateStarting( PlayerActionContext const& ctx, Vector const& stickInputVector, float stickAmplitude )
    {
    ...
    void PlayerAction_Move::RequestMoving( PlayerActionContext const& ctx, Vector const& initialVelocity )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();
    ...
    void PlayerAction_Move::UpdateMoving( PlayerActionContext const& ctx, Vector const& stickInputVectorWS, float stickAmplitude )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();
        ...
        else
        {
            ...
            if ( ctx.m_pPlayerState->m_sprintFlag )
            {
            ...
            {
                float const characterSpeed = ctx.m_pPlayer->GetVelocity().GetLength2();
                if ( characterSpeed > 1.0f && ctx.m_pInput->m_sprint.WasPressed() )
                {
                    ctx.m_pPlayerState->m_sprintFlag = true;
            ...
            float const speed = ConvertStickAmplitudeToSpeed( ctx, stickAmplitude );
            float const maxAngularVelocity = Math::DegreesToRadians * ctx.m_pPlayerState->GetAngularVelocityLimit( speed );
            ...
            m_desiredFacing = m_desiredMovementVelocity.IsZero2() ? ctx.m_pPlayer->GetForwardVector() : m_desiredMovementVelocity.GetNormalized2();
    ...
    void PlayerAction_Move::RequestStop( PlayerActionContext const& ctx )
    {
        Transform const characterWorldTransform = ctx.m_pPlayer->GetWorldTransform();
    ...
    void PlayerAction_Move::UpdateStopping( PlayerActionContext const& ctx, Vector const& stickInputVectorWS, float stickAmplitude )
    {
    ...
    #if EE_DEVELOPMENT_TOOLS
    void PlayerAction_Move::DrawDebugUI()
    {
