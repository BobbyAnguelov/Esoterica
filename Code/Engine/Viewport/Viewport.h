...
//-------------------------------------------------------------------------
//
// This is a generic abstraction of a viewport, all actual render resources and details are in "RenderViewport"
// 
...
namespace EE
{
    ...
    public:
        ...
        Viewport( Viewport const& rhs ) { operator=( rhs ); }
        Viewport( Viewport&& rhs ) { operator=( eastl::move( rhs ) ); }
        ...
        virtual bool IsValid() const { return m_ID.IsValid() && ( m_size.m_x > 0 && m_size.m_y > 0 ); }
        ...
        inline Float2 GetCenterPosition() const { return m_topLeftPosition + ( m_size / 2 ); }
        ...
        // Visualization / Debug Drawing / Tools
        //-------------------------------------------------------------------------

        #if EE_DEVELOPMENT_TOOLS
        [[nodiscard]] DebugDrawContext GetDebugDrawContext() { return m_debugDrawSystem.GetDebugDrawContext(); }
        [[nodiscard]] DebugDrawSystem* GetDebugDrawSystem() { return &m_debugDrawSystem; }

        void CreateViewportSettings( TypeSystem::TypeRegistry& typeRegistry );
        void DestroyViewportSettings();

        TVector<IViewportSettings*>& GetAllViewportSettings() { return m_settings; }
        TVector<IViewportSettings*> const& GetAllViewportSettings() const { return m_settings; }

        template<typename T>
        T* GetViewportSettings()
        {
            static_assert( std::is_base_of<IViewportSettings, T>::value, "T is not derived from ViewportVisualizationSettings" );

            for ( auto pSettings : m_settings )
            {
                T* pFoundSettings = TryCast<T>( pSettings );
                if ( pFoundSettings != nullptr )
                {
        ...
        {
            return const_cast<Viewport*>( this )->GetViewportSettings<T>();
        }

        PickingData& GetPickingData() { return m_pickingData; }
    ...
    protected:
        ...
        TVector<IViewportSettings*>                 m_settings;
