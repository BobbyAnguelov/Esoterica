...
namespace EE
{
    ...
    public:
        ...
        virtual Category GetCategory() const { return Category::Engine; }
        virtual char const* GetMenuPath() const { return nullptr; }
        virtual bool HasMenuOptions() const { return GetMenuPath() != nullptr; }
    ...
    protected:
        ...
        virtual void DrawOverlayUI( EntityWorldUpdateContext const& context ) {}

        // Does this view have any open window
        bool IsAnyWindowOpen() const;

        // Check if a window with this typeID exists
        Window* GetWindow( StringID typeID );

        // Check if a window with this userdata exists
        Window* GetWindow( uint64_t userData );

        // Check if a window with this typeID and userdata exists
        Window* GetWindow( StringID typeID, uint64_t userData );

        // Check if a window with this typeID exists
        inline bool HasDebugWindow( StringID typeID ) { return GetWindow( typeID ) != nullptr; }

        // Check if a window with this userdata exists
        inline bool HasDebugWindow( uint64_t userData ) { return GetWindow( userData ) != nullptr; }

        // Check if a window with this typeID and userdata exists
        inline bool HasDebugWindow( StringID typeID, uint64_t userData ) { return GetWindow( typeID, userData ) != nullptr; }
