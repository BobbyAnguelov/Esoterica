...
namespace EE::Animation
{
    ...
    struct TaskContext
    {
        ...
        #if EE_DEVELOPMENT_TOOLS
        // Log a task system warning
        void LogWarning( SourcePath const &sourcePath, char const *pFormat, ... ) const;

        // Log a task system error
        void LogError( SourcePath const &sourcePath, char const *pFormat, ... ) const;
        #endif
    ...
    public:
        ...
        TInlineVector<PoseTask*, 2>     m_dependencies = { nullptr, nullptr };
        ...
        bool                            m_sampleFloatChannels = true;

        #if EE_DEVELOPMENT_TOOLS
        mutable TVector<TaskLogEntry>*  m_pLog = nullptr;
        #endif
    };

    //-------------------------------------------------------------------------

    class EE_ENGINE_API PoseTask : public IReflectedType
    {
        EE_REFLECT_TYPE( PoseTask );
    ...
    public:
        ...
        #if EE_DEVELOPMENT_TOOLS
        static void DrawSecondaryPoses( DebugDrawContext& drawingContext, Transform const& worldTransform, Skeleton::LOD lod, PoseBuffer const* pRecordedPoseBuffer, bool isDetailedViewEnabled );
        #endif

    public:

        PoseTask( TaskUpdateStage updateStage = TaskUpdateStage::Any, TaskDependencies const& dependencies = TaskDependencies() );
        virtual ~PoseTask() {}
        
        // Run the task and modify the pose buffer
        ...
        virtual int32_t GetNumDependencies() const { return 0; }
        ...
        virtual void Serialize( TaskSerializer& serializer ) const {}

        // Deserialize the task from a compressed format
        virtual void Deserialize( TaskSerializer& serializer ) {}

        // Debug
        //-------------------------------------------------------------------------

        #if EE_DEVELOPMENT_TOOLS
        inline Microseconds GetExecutionTime() const { return m_executionTime; }
        inline SourcePath const& GetSourcePath() const { return m_sourcePath; }
        ...
        Transform const &GetDebugWorldTransform() const { return m_debugWorldTransform; }
        virtual void DrawDebug( DebugDrawContext& drawingContext, Transform const& worldTransform, Skeleton::LOD lod, PoseBuffer const* pRecordedPoseBuffer, bool isDetailedViewEnabled ) const;
        #endif
    ...
    protected:
        ...
        #if EE_DEVELOPMENT_TOOLS
        Transform                       m_debugWorldTransform;
