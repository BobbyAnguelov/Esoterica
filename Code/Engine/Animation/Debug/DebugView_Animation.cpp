...
namespace EE::Animation
{
    ...
    void AnimationDebugView::DrawGraphActiveTasksDebugView( GraphInstance* pGraphInstance, SourcePath const& filterPath, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        ...
        auto pTaskSystem = pGraphInstance->m_graphContext.GetTaskSystem();
        ...
        float const itemWindowHeight = ( ImGui::GetTextLineHeight() * 4 ) + ( ImGui::GetStyle().ItemSpacing.y * 3 ) + ( ImGui::GetStyle().WindowPadding.y * 2 );
        ...
        ImVec2 const windowSize( ImGui::GetContentRegionAvail().x, layout.m_dimensions.m_y + itemWindowHeight );
        if ( ImGui::BeginChild( "TL", windowSize, ImGuiChildFlags_AlwaysUseWindowPadding, ImGuiWindowFlags_HorizontalScrollbar ) )
        {
            ...
            TInlineVector<SourcePath, 20> sourcePaths;
            for ( int8_t i = 0; i < numTasks; i++ )
            {
                sourcePaths.emplace_back( pTaskSystem->m_tasks[i]->GetSourcePath() );
            }
            ...
            for ( TreeLayout::Node_t& node : layout.m_nodes )
            {
                ...
                for ( int32_t j = 0; j < numDependencies; j++ )
                {
                    ...
                    if ( filterPath.IsValid() && !filterPath.IsUnderOrEqual( sourcePaths[taskIdx] ) )
                    {
            ...
            for ( int8_t i = 0; i < numTasks; i++ )
            {
                ...
                auto pTask = ( PoseTask const* ) node.m_pItem;

                // Is this item from the current debug instance?
                bool wasCreatedByCurrentDebugInstance = true;
                if ( filterPath.IsValid() && !filterPath.IsUnderOrEqual( sourcePaths[i] ) )
                {
                    wasCreatedByCurrentDebugInstance = false;
                }
                ...
                if ( drawChild )
                {
                    ...
                    ImGui::Text( "Execution Time: %.2fus", pTask->GetExecutionTime().ToFloat() );
                    ...
                    {
                        InlineString const sourcePathStr = pGraphInstance->ResolveSourcePath( sourcePaths[i] );
                        ImGui::SetTooltip( sourcePathStr.c_str() );
                        ...
                        if ( navigateToNodeFunc != nullptr )
                        {
                            ...
                            {
                                navigateToNodeFunc( sourcePaths[i] );
                            }
                        }
                    }
                }
    ...
    void AnimationDebugView::DrawRootMotionRow( GraphInstance* pGraphInstance, SourcePath const& filterPath, RootMotionDebugger const* pRootMotionRecorder, int16_t currentActionIdx, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        ...
        if ( pAction != nullptr )
        {
            SourcePath const sourcePath = pRootMotionRecorder->GetRecordedActions()[currentActionIdx].m_sourcePath;

            bool wasSampledFromDebuggedInstance = true;
            if ( filterPath.IsValid() && !filterPath.IsUnderOrEqual( sourcePath ) )
            {
            ...
            InlineString const sourcePathStr = pGraphInstance->ResolveSourcePath( sourcePath );
            InlineString const nodeName( InlineString::CtorSprintf(), "%s##%d_%s", RootMotionDebugger::s_actionTypeLabels[(int32_t) pAction->m_actionType], currentActionIdx, sourcePathStr.c_str() );
            ...
            if ( wasSampledFromDebuggedInstance )
            {
                if ( navigateToNodeFunc != nullptr )
                {
                    ...
                    {
                        navigateToNodeFunc( sourcePath );
                    }
                    ImGuiX::ItemTooltip( sourcePathStr.c_str() );
                    ImGui::PopID();
                }
            }
            else
            {
                ...
                ImGuiX::TextTooltip( "Sampled from another instance: %s", sourcePathStr.c_str() );
            }
    ...
    void AnimationDebugView::DrawRootMotionDebugView( GraphInstance* pGraphInstance, SourcePath const& filterPath, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        if ( pGraphInstance == nullptr || !pGraphInstance->IsInitialized() )
        {
    ...
    void AnimationDebugView::DrawSampledAnimationEventsView( GraphInstance* pGraphInstance, SourcePath const& filterPath, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        if ( pGraphInstance == nullptr || !pGraphInstance->IsInitialized() )
        {
        ...
        SampledEventsBuffer const& sampledEventsBuffer = pGraphInstance->GetSampledEvents();

        //-------------------------------------------------------------------------

        auto DrawEventRow = [&] ( SampledEvent const& sampledEvent )
        {
            SourcePath const& sourcePath = sampledEvent.GetSourcePath();
            ...
            // Note: this is really naive and a clear place for optimization if needed
            if ( VectorContains( sampledEventsBuffer.GetNewlyReceivedEvents(), sampledEvent ) )
            {
                ImGui::TextColored( Colors::Lime.ToFloat4(), "New" );
            }
            else if ( VectorContains( sampledEventsBuffer.GetContinuousEvents(), sampledEvent ) )
            {
                ImGui::TextColored( Colors::Yellow.ToFloat4(), "Continuing" );
            }
            else if ( VectorContains( sampledEventsBuffer.GetEndedContinuousEvents(), sampledEvent ) )
            {
                ImGui::TextColored( Colors::OrangeRed.ToFloat4(), "Ended" );
            }
            else
            {
                ImGui::TextColored( Colors::Gray.ToFloat4(), "Unknown" );
            }
            ...
            ImGui::Text( pEvent->GetTypeInfo()->m_friendlyName.c_str() );
            ImGuiX::TextTooltip( pEvent->GetTypeInfo()->m_friendlyName.c_str() );

            // Value
            //-------------------------------------------------------------------------

            ImGui::TableNextColumn();
            InlineString const debugString = pEvent->GetDebugText( sampledEvent.GetPercentageThrough() );
            ...
            InlineString const pathStr = pGraphInstance->ResolveSourcePath( sourcePath );
            if ( wasSampledFromDebuggedInstance )
            {
                if ( navigateToNodeFunc != nullptr )
                {
                    ...
                    ImGuiX::ItemTooltip( pathStr.c_str() );
                }
                else
                {
                    ImGui::SameLine( 14.0f );
                    ImGui::Text( EE_ICON_INFORMATION );
                    ImGuiX::TextTooltip( pathStr.c_str() );
                }
            }
            else
            {
                ImGui::SameLine( 14.0f );
                ImGui::Text( EE_ICON_INFORMATION );
                ImGuiX::TextTooltip( "Sampled from another instance: %s", pathStr.c_str() );
            }
        };

        //-------------------------------------------------------------------------

        if ( ImGui::BeginTable( "AnimEventsTable", 8, ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable ) )
        {
            ...
            ImGui::TableSetupColumn( "Weight", ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, 60 );
            ...
            for ( int32_t i = 0; i < sampledEventsBuffer.GetNumSampledEvents(); i++ )
            {
                SampledEvent const& sampledEvent = sampledEventsBuffer.GetEvent( i );
                ...
                ImGui::PushID( i );
                DrawEventRow( sampledEvent );
                ImGui::PopID();
            }

            //-------------------------------------------------------------------------

            for ( auto const& sampledEvent : sampledEventsBuffer.GetEndedContinuousEvents() )
            {
                if ( sampledEvent.IsGraphEvent() )
                {
                    continue;
    ...
    void AnimationDebugView::DrawSampledGraphEventsView( GraphInstance* pGraphInstance, SourcePath const& filterPath, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        if ( pGraphInstance == nullptr || !pGraphInstance->IsInitialized() )
        {
            ImGui::Text( "Nothing to Show!" );
        ...
        if ( ImGui::BeginTable( "GraphEventsTable", 6, ImGuiTableFlags_Borders | ImGuiTableFlags_Resizable ) )
        {
            ImGui::TableSetupColumn( "Branch", ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, 18 );
            ImGui::TableSetupColumn( "Ignored", ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, 18 );
            ImGui::TableSetupColumn( "Status", ImGuiTableColumnFlags_WidthFixed | ImGuiTableColumnFlags_NoResize, 40 );
    ...
    void AnimationDebugView::DrawCombinedSampledEventsView( GraphInstance* pGraphInstance, SourcePath const& filterPath, NavigateToSourceFunc const& navigateToNodeFunc )
    {
        if ( pGraphInstance == nullptr || !pGraphInstance->IsInitialized() )
        {
            ImGui::Text( "Nothing to Show!" );
    ...
    void AnimationDebugView::DrawFloatCurvesDebugView( GraphInstance* pGraphInstance )
    {
        //THashMap<StringID, float> floatCurves;
        //pGraphInstance->SampleFloatCurves( floatCurves );
        ...
        //    for( TPair<StringID, float> const& pair : floatCurves )
        //    {
        //        ImGui::TableNextRow();

        //        ImGui::TableNextColumn();
        //        ImGui::Text( pair.first.IsValid() ? pair.first.c_str() : "Invalid ID" );

        //        ImGui::TableNextColumn();
        //        ImGui::TextColored( Color::EvaluateRedGreenGradient( pair.second ).ToFloat4(), "%.5f", pair.second );
        //    }
    ...
    void AnimationDebugView::DrawMenu( EntityWorldUpdateContext const& context )
    {
        ...
        static StringID const floatCurvesWindowID( "Float Curves" );
        ...
        for ( GraphComponent* pGraphComponent : m_pAnimationWorldSystem->m_graphComponents )
        {
            ...
            {
                ImGui::MenuItem( componentName.c_str(), nullptr, nullptr, false );
                continue;
            }
            ...
            if ( ImGui::BeginMenu( componentName.c_str() ) )
            {
                ...
                {
                    auto pTaskWindowForComponent = GetWindow( controlParameterWindowID, pGraphComponent->GetID().m_value );
                ...
                {
                    auto pTaskWindowForComponent = GetWindow( eventsWindowID, pGraphComponent->GetID().m_value );
                    if ( pTaskWindowForComponent != nullptr )
                    {
                        pTaskWindowForComponent->m_isOpen = true;
                    }
                ...
                if ( ImGui::MenuItem( "Show Sampled Float Curves" ) )
                {
                    auto pTaskWindowForComponent = GetWindow( floatCurvesWindowID, pGraphComponent->GetID().m_value );
                    if ( pTaskWindowForComponent != nullptr )
                    {
                        pTaskWindowForComponent->m_isOpen = true;
                    }
                    ...
                    {
                        InlineString windowName( InlineString::CtorSprintf(), "Sampled Float Curves: %s", componentName.c_str() );
                        m_windows.emplace_back( windowName.c_str(), [this] ( EntityWorldUpdateContext const& context, bool isFocused, uint64_t userData ) { DrawFloatCurvesWindow( context, isFocused, userData ); } );
                        m_windows.back().m_typeID = floatCurvesWindowID;
                        m_windows.back().m_userData = pGraphComponent->GetID().m_value;
                        m_windows.back().m_isOpen = true;
                    }
                }
                ...
                {
                    auto pTaskWindowForComponent = GetWindow( tasksWindowID, pGraphComponent->GetID().m_value );
                    if ( pTaskWindowForComponent != nullptr )
                    {
                        pTaskWindowForComponent->m_isOpen = true;
                    }
    ...
    void AnimationDebugView::DrawFloatCurvesWindow( EntityWorldUpdateContext const& context, bool isFocused, uint64_t userData )
    {
        auto ppFoundComponent = m_pAnimationWorldSystem->m_graphComponents.FindItem( ComponentID( userData ) );
        EE_ASSERT( ppFoundComponent != nullptr );
        auto pGraphComponent = *ppFoundComponent;
        DrawFloatCurvesDebugView( pGraphComponent->m_pGraphInstance );
    }
}
#endif