...
namespace EE::Serialization
{
    ...
    public:
        ...
        // Read back a quantized float - requires you to provide a quantization range with the min/max values
        float ReadQuantizedFloat( float minPossibleValue, float maxPossibleValue );
    ...
    using BitArchive = TBitArchive<1280>;

    //-------------------------------------------------------------------------

    template<size_t N>
    void TBitArchive<N>::WriteBool( bool value )
    {
    ...
    template<size_t N>
    void TBitArchive<N>::WriteUInt( uint64_t value, uint32_t maxBitsToUse )
    {
        EE_ASSERT( !m_isReading );
    ...
    template<size_t N>
    void TBitArchive<N>::WriteNormalizedFloat8Bit( float value )
    {
        EE_ASSERT( !m_isReading );
    ...
    template<size_t N>
    void TBitArchive<N>::WriteNormalizedFloat16Bit( float value )
    {
        EE_ASSERT( !m_isReading );
    ...
    template<size_t N>
    void TBitArchive<N>::WriteQuantizedFloat( float value, float minPossibleValue, float maxPossibleValue )
    {
        EE_ASSERT( !m_isReading );
    ...
    template<size_t N>
    void TBitArchive<N>::WriteFloat( float value )
    {
        EE_ASSERT( !m_isReading );
    ...
    template<size_t N>
    void TBitArchive<N>::GetWrittenData( Blob& outData )
    {
        EE_ASSERT( !m_isReading );

        size_t const numBytesToCopy = Math::CeilingToInt32( m_bitPos / 8.0f );
    ...
    template<size_t N>
    uint64_t TBitArchive<N>::ReadUInt( uint64_t maxBitsToUse )
    {
        EE_ASSERT( m_isReading );
        uint64_t value = 0;
    ...
    template<size_t N>
    float TBitArchive<N>::ReadNormalizedFloat8Bit()
    {
        EE_ASSERT( m_isReading );
    ...
    template<size_t N>
    float TBitArchive<N>::ReadNormalizedFloat16Bit()
    {
        EE_ASSERT( m_isReading );
    ...
    template<size_t N>
    float TBitArchive<N>::ReadQuantizedFloat( float minPossibleValue, float maxPossibleValue )
    {
        EE_ASSERT( m_isReading );
        EE_ASSERT( maxPossibleValue > minPossibleValue );
        uint16_t const encodedFloat = (uint16_t) ReadUInt( 16 );
