...
namespace EE::ImGuiX
{
    ...
    void CurveEditor::DrawGridAndLegend( ImDrawList* pDrawList )
    {
        ...
        int32_t const numVerticalLines = Math::FloorToInt32( m_curveCanvasWidth / s_pixelsPerGridBlock );
        ...
        int32_t const numHorizontalLines = Math::FloorToInt32( m_curveCanvasHeight / s_pixelsPerGridBlock );
    ...
    {
        int32_t const numPointsToDraw = Math::RoundToInt32( m_curveCanvasWidth / 2 ) + 1;
    ...
    bool CurveEditor::DrawInTangentHandle( ImDrawList* pDrawList, int32_t pointIdx )
    {
        ...
        if ( ImGui::IsItemActive() && ImGui::IsMouseDragging( 0 ) )
        {
            ...
            tangentCanvasOffset = Vector( tangentCanvasOffset ).GetNormalized2().ToFloat2();
    ...
    void CurveEditor::DrawContextMenus( bool isMiniView )
    {
        if ( ImGui::BeginPopup( s_gridContextMenuName ) )
        {
            ...
            if ( ImGui::MenuItem( EE_ICON_CONTENT_COPY" Copy" ) )
            {
                ImGui::SetClipboardText( m_curve.ToString().c_str() );
            }

            if ( ImGui::MenuItem( EE_ICON_CONTENT_PASTE" Paste" ) )
            {
                String serializedCurveStr = ImGui::GetClipboardText();

                TInlineVector<FloatCurve::Point, 8> tempPoints;
                if ( !serializedCurveStr.empty() && FloatCurve::DeserializePointsFromString( serializedCurveStr, tempPoints ) )
                {
                    StartEditing();
                    m_curve.SetPoints( tempPoints );
                    StopEditing();
                }
            }
