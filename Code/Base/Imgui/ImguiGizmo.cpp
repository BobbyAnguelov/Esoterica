...
namespace EE::ImGuiX
{
    ...
    Gizmo::Result Gizmo::Draw( Vector const& positionWS, Quaternion const& orientationWS, Render::Viewport const& viewport )
    {
        ...
        offsetViewport.Resize( Float2( ImGui::GetWindowPos() ), viewport.GetDimensions() );
    ...
    static Color const                  g_hoveredAxisColorX = EE::Colors::Red;
    static Color const                  g_hoveredAxisColorY = EE::Colors::LimeGreen;
    static Color const                  g_hoveredAxisColorZ = EE::Colors::DodgerBlue;
    ...
    static Color const                  g_rotationPreviewColor = EE::Colors::Orange;
    static Color const                  g_originColor = EE::Colors::White;
    ...
    Gizmo::Result Gizmo::DrawTranslationGizmo( Vector const& originWS, Quaternion const& orientationWS, Render::Viewport const& viewport )
    {
        ...
        planeInfo[0].m_color = Colors::Yellow;
        ...
        planeInfo[1].m_color = Colors::DarkOrchid;
        ...
        planeInfo[2].m_color = Colors::LightSeaGreen;
        ...
        axisInfo[1].m_color = ( axisInfo[1].m_isHovered ) ? g_hoveredAxisColorY : g_axisColorY;
        axisInfo[2].m_color = ( axisInfo[2].m_isHovered ) ? g_hoveredAxisColorZ : g_axisColorZ;
    ...
    Gizmo::Result Gizmo::DrawRotationGizmo( Vector const& originWS, Quaternion const& orientationWS, Render::Viewport const& viewport )
    {
        ...
        axisInfo[0].m_color = g_axisColorX;
        axisInfo[0].m_isHovered = ( m_manipulationMode == axisInfo[0].m_manipulationMode );
        ...
        axisInfo[1].m_color = g_axisColorY;
        axisInfo[1].m_isHovered = ( m_manipulationMode == axisInfo[1].m_manipulationMode );
        ...
        axisInfo[2].m_color = g_axisColorZ;
        axisInfo[2].m_isHovered = ( m_manipulationMode == axisInfo[2].m_manipulationMode );
        ...
        if ( isManipulating )
        {
            ...
            if ( Math::Abs( m_rotationDeltaAngle.ToFloat() ) > Math::DegreesToRadians )
            {
                ...
                pDrawList->AddPolyline( previewCirclePointsSS, numPreviewCirclePoints, g_rotationPreviewColor, false, 8.0f );
                pDrawList->AddLine( originSS, previewCirclePointsSS[numPreviewCirclePoints - 1], g_rotationPreviewColor, 1.0f );
                pDrawList->AddLine( originSS, previewCirclePointsSS[0], Colors::White, 1.0f );
            }
            ...
            pDrawList->AddLine( originSS, pointOnCircle_SS, Colors::White, 1.0f );
            pDrawList->AddCircleFilled( pointOnCircle_SS, 3.0f, g_rotationPreviewColor );

            auto textSize = ImGui::CalcTextSize( buff );
            Vector const textPosition = Vector( pointOnCircle_SS ) - Float2( textSize.x / 2, 0.0f );
            pDrawList->AddRectFilled( textPosition, textPosition + Float2( textSize.x + 3, textSize.y ), EE::Colors::Black.GetAlphaVersion( 0.5f ), 3.0f );
            pDrawList->AddText( textPosition, Colors::White, buff );
        }
